 ...
 * ```
 */
const useHovercardAnchor = system.createHook(_ref => {
  let {
    state,
    showOnHover = true,
    ...props
  ...
  const showTimeoutRef = react.useRef(0); // Clear the show timeout if the anchor is unmounted

  react.useEffect(() => () => window.clearTimeout(showTimeoutRef.current), []); // Clear the show timeout if the mouse leaves the anchor element. We're
  ...
  react.useEffect(() => {
    ...
    return events.addGlobalEventListener("mouseleave", onMouseLeave, true);
  }, [state.anchorRef]);
  const onMouseMoveProp = hooks.useEventCallback(props.onMouseMove);
  const showOnHoverProp = hooks.useBooleanEventCallback(showOnHover);
...
const HovercardAnchor = system.createComponent(props => {
  const htmlProps = useHovercardAnchor(props);
  return system.createElement("a", htmlProps);
});

exports.HovercardAnchor = HovercardAnchor;
exports.useHovercardAnchor = useHovercardAnchor;
