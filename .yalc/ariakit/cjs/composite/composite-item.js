...
const useCompositeItem = system.createHook(_ref => {
  ...
  const onFocus = react.useCallback(event => {
    ...
    if (!events.isSelfTarget(event)) return; // and the composite item is not a text field or contenteditable
    // element.
  ...
  const preventScrollOnKeyDownProp = hooks.useBooleanEventCallback(preventScrollOnKeyDown);
