...
function useDisableEvent(eventProp, disabled) {
  const onEventProp = hooks.useEventCallback(eventProp);
...
const useFocusable = system.createHook(_ref => {
  let {
    focusable = true,
  ...
  const ref = react.useRef(null); // Add global event listeners to determine whether the user is using a
  // keyboard to navigate the site or not.

  react.useEffect(() => {
    if (!focusable) return;
    events.addGlobalEventListener("mousedown", onGlobalMouseDown, true);
    events.addGlobalEventListener("keydown", onGlobalKeyDown, true);
  ...
  if (isSafariOrFirefoxOnAppleDevice) {
    react.useEffect(() => {
      ...
      const onMouseUp = () => misc.queueMicrotask(() => element.focus());
  ...
  const [focusVisible, setFocusVisible] = react.useState(false); // When the focusable element is disabled, it doesn't trigger a blur event
  ...
  const onMouseDown = react.useCallback(event => {
    ...
    if (!dom.isButton(element) && !isNativeCheckboxOrRadio(element)) return; // We can't focus right away after on mouse down, otherwise it would
    ...
    events.queueBeforeEvent(element, "mouseup", () => focus.focusIfNeeded(element));
  }, [onMouseDownProp, focusable]);
  const onFocusVisibleProp = hooks.useEventCallback(onFocusVisible);
  const onFocusVisibleEvent = react.useCallback(event => {
  ...
  const onBlurProp = hooks.useEventCallback(props.onBlur); // Note: Can't use onBlurCapture here otherwise it will not work with
...
const Focusable = system.createComponent(props => {
  props = useFocusable(props);
  return system.createElement("div", props);
});
