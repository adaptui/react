...
function usePopoverState(_temp) {
  ...
  const [anchorRect, setAnchorRect] = hooks.useControlledState(props.defaultAnchorRect || null, props.anchorRect, props.setAnchorRect);
  const anchorRef = react.useRef(null);
  const popoverRef = react.useRef(null);
  const arrowRef = react.useRef(null);
  const [currentPlacement, setCurrentPlacement] = react.useState(placement);
  const [rendered, render] = react.useReducer(() => ({}), {});
