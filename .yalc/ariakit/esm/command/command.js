...
import { isSelfTarget, queueBeforeEvent, fireClickEvent } from 'ariakit-utils/events';
import { useTagName, useEventCallback, useForkRef } from 'ariakit-utils/hooks';
...
const useCommand = createHook(_ref => {
  let {
    clickOnEnter = true,
    clickOnSpace = true,
    ...props
    ...
    if (isEnter || isSpace) {
      ...
      if (isEnter) {
        if (!nativeClick) {
          ...
          queueBeforeEvent(element, "keyup", () => // Fire a click event instead of calling element.click()
...
const Command = createComponent(props => {
