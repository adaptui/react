...
import { useForkRef, useBooleanEventCallback, useWrapElement, useEventCallback } from 'ariakit-utils/hooks';
...
function useAutoFocusOnShow(_ref) {
  ...
  useEffect(() => () => state.setAutoFocusOnShow(false), [state.setAutoFocusOnShow]);
  return {
...
const useHovercard = createHook(_ref3 => {
  ...
  const hideOnControlProp = useBooleanEventCallback(hideOnControl); // Hide on Escape/Control. Popover already handles this, but only when the
  ...
  const disablePointerEventsProp = useBooleanEventCallback(disablePointerEventsOnApproach); // Checks whether the mouse is moving toward the hovercard. If not, hide the
