...
function getAutocomplete(options: ComboboxOptions) {
...
function isFirstItemAutoSelected(
  items: ComboboxOptions["items"],
  autoSelect: ComboboxOptions["autoSelect"],
  currentId: ComboboxOptions["currentId"],
) {
...
function useValue(options: ComboboxOptions) {
...
function getFirstEnabledItemId(items: ComboboxOptions["items"]) {
  return items.find(item => !item.disabled)?.id;
}

export const useCombobox = createHook<ComboboxOptions, ComboboxHTMLProps>({
  name: "Combobox",
...
export type ComboboxOptions = CompositeOptions &
  Pick<
  ...
  > &
  Pick<ComboboxStateReturn, "baseId" | "inputValue" | "setInputValue"> & {
    /**
...
export type ComboboxHTMLProps = CompositeHTMLProps &
  React.InputHTMLAttributes<any>;

export type ComboboxProps = ComboboxOptions & ComboboxHTMLProps;
