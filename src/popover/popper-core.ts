...
function getAlignAccountingForCollisions(
  ...
  if (align === "start" || align === "center") {
    if (
      (collisions[startBound] && isAnchorBigger) ||
      (collisions[endBound] && !isAnchorBigger)
    ) {
      return "end";
    }
  }
...
function getTransformOrigin(
  ...
  if (isHorizontalSide) {
    ...
    y =
      side === "top"
        ? `${popperSize.height + sideOffset}px`
        : `${-sideOffset}px`;
  } else {
    x =
      side === "left"
        ? `${popperSize.width + sideOffset}px`
        : `${-sideOffset}px`;
...
export { ALIGN_OPTIONS, getPlacementData, SIDE_OPTIONS };
